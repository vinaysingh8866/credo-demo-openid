...
export class HolderInquirer extends BaseInquirer {
  ...
  private async getPromptChoice() {
    ...
    if (this.resolvedCredentialOffer)
      promptOptions.push(PromptOptions.RequestCredential);
    if (this.resolvedPresentationRequest)
      promptOptions.push(PromptOptions.AcceptPresentationRequest);
  ...
  public async resolveCredentialOffer() {
    const credentialOffer = await prompt([
      this.inquireInput("Enter credential offer: "),
    ]);
    const resolvedCredentialOffer = await this.holder.resolveCredentialOffer(
      credentialOffer.input
    );
    this.resolvedCredentialOffer = resolvedCredentialOffer;

    console.log(
      greenText(`Received credential offer for the following credentials.`)
    );
    console.log(
      greenText(
  ...
  public async requestCredential() {
    ...
    const credentialsThatCanBeRequested =
      this.resolvedCredentialOffer.offeredCredentials.map(
        (credential) => credential.id
      );

    const choice = await prompt([
      this.inquireOptions(credentialsThatCanBeRequested),
    ]);

    const credentialToRequest =
      this.resolvedCredentialOffer.offeredCredentials.find(
        (credential) => credential.id === choice.options
      );
    if (!credentialToRequest)
      throw new Error("Credential to request not found.");

    console.log(
      greenText(
        `Requesting the following credential '${credentialToRequest.id}'`
      )
    );
    console.log(this.resolveCredentialOffer, "fetching credentials");
  ...
  public async resolveProofRequest() {
    const proofRequestUri = await prompt([
      this.inquireInput("Enter proof request: "),
    ]);
    this.resolvedPresentationRequest = await this.holder.resolveProofRequest(
      proofRequestUri.input
    );

    const presentationDefinition =
      this.resolvedPresentationRequest?.presentationExchange?.definition;
    console.log(
      greenText(`Presentation Purpose: '${presentationDefinition?.purpose}'`)
    );

    if (
      this.resolvedPresentationRequest?.presentationExchange
      ...
      console.log(
        redText(`No credentials available that satisfy the proof request.`)
      );
    }
  }
  ...
  public async acceptPresentationRequest() {
    if (!this.resolvedPresentationRequest)
      throw new Error("No presentation request resolved yet.");

    console.log(greenText(`Accepting the presentation request.`));

    const serverResponse = await this.holder.acceptPresentationRequest(
      this.resolvedPresentationRequest
    );
    ...
    if (serverResponse.status >= 200 && serverResponse.status < 300) {
      ...
      console.log(
        ...
        }'. ${JSON.stringify(serverResponse.body)}`
      );
    }
  }
  ...
  private printCredential = (
    credential: W3cCredentialRecord | SdJwtVcRecord
  ) => {
    if (credential.type === "W3cCredentialRecord") {
      console.log(
        greenText(
          `W3cCredentialRecord with claim format ${credential.credential.claimFormat}`,
          true
        )
      );
      console.log(
        JSON.stringify(credential.credential.jsonCredential, null, 2)
      );
      console.log("");
    } else {
      console.log(greenText(`SdJwtVcRecord`, true));
      const prettyClaims = this.holder.agent.sdJwtVc.fromCompact(
